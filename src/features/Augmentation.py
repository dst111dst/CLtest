...
class CombinatorialEnumerate(object):
    ...
    def __init__(self,item_similarity_model,tao=0.8,\
                 gamma=0.5, beta=0.5, \
                 insert_rate=0.4, \
...
class Random(object):
    ...
    """
    def __init__(self,item_similarity_model, tao=0.2, \
                 gamma=0.7, beta=0.2,insert_rate=0.3, \
...
class Crop(object): # 裁剪，但是可能会产生比较大的...
    """
    Randomly crop a sub-sequence from the original sequence
    """
    def __init__(self, tao=0.7):
        self.tao = tao

    def __call__(self, sequence):
        """
        : param: sequence -> tensor.
        : return: Sequence being cropped.
        """
        copied_sequence = copy.deepcopy(sequence)
...
class Reorder(object):
    """
    Randomly shuffle a continuous sub-sequence. It may harm the sequence-like data.
    """
...
if __name__ == '__main__':
    ...
    enum_type = Random(item_similarity_model=feature_based_similarity)
    enum_type = CombinatorialEnumerate(n_views=n_views,item_similarity_model=feature_based_similarity)
    # print(enum_type)
    for i in range(40):
        try:
